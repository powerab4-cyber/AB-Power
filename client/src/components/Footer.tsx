...
const colorMap: Record<string, string> = {
  pink: 'hover:border-pink-500/30 hover:bg-pink-500/10 hover:text-pink-400 hover:shadow-pink-500/10',
  cyan: 'hover:border-cyan-400/30 hover:bg-cyan-400/10 hover:text-cyan-400 hover:shadow-cyan-400/10',
  blue: 'hover:border-blue-500/30 hover:bg-blue-500/10 hover:text-blue-400 hover:shadow-blue-500/10',
  green: 'hover:border-green-500/30 hover:bg-green-500/10 hover:text-green-400 hover:shadow-green-500/10',
}

export function Footer() {
  return (
    <footer id="contact" className="relative border-t border-white/5 py-10 sm:py-12">
      ...
      <div className="relative mx-auto max-w-7xl px-5 sm:px-6">
        <div className="flex flex-col items-center gap-8">
          ...
          {/* Nav links — 2 cols phone, 4 cols tablet+ */}
          <nav className="grid w-full grid-cols-2 gap-2 text-sm text-mist sm:grid-cols-4 sm:gap-3">
            {footerLinks.map((link) =>
              link.to.startsWith('#') ? (
                <a
                  key={link.to}
                  href={link.to}
                  className="rounded-xl border border-white/5 bg-white/[0.03] py-3 text-center transition-colors hover:text-white"
                >
                  {link.label}
                </a>
              ) : (
          ...
          {/* Social links — 2 cols phone, 4 cols tablet+ */}
          <div className="grid w-full grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-3">
            {socialLinks.map((s) => (
              <a
                ...
                className={`group flex items-center justify-center gap-2.5 rounded-xl border border-white/5 bg-white/[0.03] py-3.5 text-sm font-medium text-mist backdrop-blur-sm transition-all duration-300 hover:shadow-lg ${colorMap[s.color]}`}
                aria-label={s.label}
              >
          ...
          {/* Address */}
          <div className="flex w-full max-w-sm flex-col items-center gap-2 rounded-2xl border border-white/5 bg-white/[0.03] px-5 py-4 text-center">
